...
import { clearWorkoutCache, decodeAthleteFromSource, deleteWorkout, loadWorkouts, saveWorkout, stripAthleteFromSource, type SharedWorkout } from './services/supabase';
...
function App() {
  ...
  const handleDeleteWorkout = async (id: string, name: string) => {
    ...
    try {
      ...
      await refreshCoachAdvice(mapped, geminiApiKey);
      setFormStatus('Allenamento eliminato.');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Impossibile eliminare. Riprova.'
